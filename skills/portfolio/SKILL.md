...
**`caption`/`title` MUST describe the actual post — never fabricate (issue #7).** The `caption` and `title` are the only social fields you author, and a wrong one is a **correctness bug, not a cosmetic one**: a caption sitting next to an embed of a *different* post (e.g. a guitar-practice reel labeled "AI at EY's Global Summit") destroys trust the moment the reader notices. Get the words from the post itself — the browser-pull path, the URL slug, or by asking the user — and write only what the post actually says. **On regeneration / when reusing a prior `data.js`, re-verify every carried-over `caption`/`title` against the post the URL now points to** before shipping; stale captions from an earlier build are the most common source of this bug. When unsure, ship the post with no `caption` rather than a guessed one.
