...
Let the user choose. Their energy matters — a project they want to build ships; one you assigned them stalls. If they pick something other than your recommendation, take it; you'll spec *that* one.
...
Write each project as a **`Project` node** (the same `Project` type `hope-portfolio` already understands) with these fields:
...
1. **When they ship → `hope-portfolio`.** "When you ship it, I'll add it to your portfolio." A shipped `Project` node (status flipped from planned, live link + real numbers filled in) is exactly the kind of real evidence `hope-portfolio` features — re-run the portfolio and it decides to include the now-shipped project. (Ownership is a `HAS_PROJECT` edge; the portfolio makes its own inclusion call at generation time — it isn't automatic.)
